...
while True:
    ...
    # рисуем сетку
    [pygame.draw.line(
        polygon, pygame.Color('darkslategray'), (x, 0), (x, HEIGHT))
        for x in range(0, WIDTH, NAME)]
    [pygame.draw.line(
        polygon, pygame.Color('darkslategray'), (0, y), (WIDTH, y))
        for y in range(0, HEIGHT, NAME)]
    # строим жизнь
    for x in range(1, width-1):
        for y in range(1, height-1):
            if current_field[y][x]:
                pygame.draw.rect(
                    polygon, pygame.Color('green2'),
                    (x*NAME+2, y*NAME+2, NAME-2, NAME-2))
