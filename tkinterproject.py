...
class GameOfLife(Frame):
    ...
    def initialUI(self):
        ...
        prompt = Label(
            self.title_frame,
            text="Click the cells to create the starting configuration,"
            "press Start Game: ")
    ...
    def build_grid(self):
        ...
        self.game_frame = Frame(
            self.parent, width=self.size_x + 2,
            height=self.size_y + 2, borderwidth=1, relief=SUNKEN)
        self.game_frame.grid(row=2, column=0, columnspan=4)

        # instantiates buttons for choosing initial configuration
        self.cell_buttons = [
            [Button(self.game_frame, bg="white", width=2, height=1)
        ...
        for i in range(1, self.size_y + 1):
            for j in range(1, self.size_x + 1):
                ...
                self.cell_buttons[i][j]['command'] = lambda i=i, j=j: \
                    self.cell_toggle(
    ...
    def simulate_game(self):
        ...
        for i in range(1, self.size_y + 1):
            for j in range(1, self.size_x + 1):
                coord = (i, j)
                # if cell dead and has 3 neighbors,
                # add coordinate to list of coords to toggle
                if self.cell_buttons[i][j]['bg'] == "white" \
                and self.neighbor_count(i, j) == 3:
                    buttons_to_toggle.append(coord)
                # if cell alive and does not have 2 or 3 neighbors,
                # add coordinate to list of coords to toggle
                elif self.cell_buttons[i][j]['bg'] == "black" \
                and self.neighbor_count(i, j) != 3\
    ...
    def neighbor_count(self, x_coord, y_coord):
        ...
        for i in range(x_coord - 1, x_coord + 2):
            for j in range(y_coord - 1, y_coord + 2):
                ...
                and self.cell_buttons[i][j]['bg'] == "black":
                    count += 1
